...
class Bottles {
   ...
   */
  verseN(verseNumber) {
    let bottlePluralizationStr = verseNumber === 2 ? "bottle" : "bottles";
    return (
      `${verseNumber} bottles of beer on the wall, ` +
      `${verseNumber} bottles of beer.\n` +
      "Take one down and pass it around, " +
      `${verseNumber - 1} ${bottlePluralizationStr} of beer on the wall.\n`
    );
  }
  ...
  verseEnd(verseNumber) {
    let bottlePluralizationStr =
      verseNumber === 1 ? "1 bottle" : "No more bottles";

    let line =
      `${bottlePluralizationStr} of beer on the wall, ` +
      `${bottlePluralizationStr.toLowerCase()} of beer.\n`;
